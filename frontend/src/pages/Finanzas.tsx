...
import { autoImportarGastos } from '@/utils/gastos-import'
...
function SeccionGastos() {
  ...
  useEffect(() => {
    ...
    autoImportarGastos()
      .then(res => {
        if (!res) { setSyncEstado('idle'); return }
        setSyncEstado('ok')
        setSyncMsg(res.importados > 0
          ? `${res.importados} nuevo${res.importados !== 1 ? 's' : ''} importado${res.importados !== 1 ? 's' : ''}`
          : 'Al día')
        if (res.importados > 0) cargar()
      })
      .catch(() => { setSyncEstado('error'); setSyncMsg('Error al sincronizar') })
  }, [])
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {syncEstado === 'syncing' && (
            <span className="inline-flex items-center gap-1.5 text-xs text-slate-500 px-2.5 py-1 rounded-full bg-slate-100 border border-slate-200">
              <RefreshCw className="w-3 h-3 animate-spin" /> Sincronizando...
            </span>
          )}
          {syncEstado === 'ok' && (
            <span className="inline-flex items-center gap-1.5 text-xs text-[#4a7030] px-2.5 py-1 rounded-full bg-[#edf5e0] border border-[#c0d8a0]">
              <CheckCircle2 className="w-3 h-3" /> {syncMsg}
            </span>
          )}
          {syncEstado === 'error' && (
            <span className="inline-flex items-center gap-1.5 text-xs text-amber-700 px-2.5 py-1 rounded-full bg-amber-50 border border-amber-200">
              <AlertCircle className="w-3 h-3" /> {syncMsg}
            </span>
          )}
